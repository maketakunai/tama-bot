...
exports.run = (client, message, args) => {
  if (args.length == 0){
    ...
    let results = calcExp(args[0], args[1]);
...
function calcExp(x, y){
  let totalExp = expChart[y-1].Exp_Total - expChart[x-1].Exp_Total;
  let normExp = Math.ceil(totalExp/exp_normal);
  let bonusExp = Math.ceil(totalExp/exp_bonus);
  let normExp5 = Math.ceil(totalExp/(exp_normal*3));
  let bonusExp5 = Math.ceil(totalExp/(exp_bonus*3));
